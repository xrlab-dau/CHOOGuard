...
namespace ChooGuard.App.Mvp
{
    public sealed class MvpAgencyDispatchView : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            panel=new GameObject("AgencyDispatchPanel",typeof(RectTransform),typeof(Image));panel.transform.SetParent(canvas,false);var r=(RectTransform)panel.transform;Rect(r,24,72,320,440);panel.GetComponent<Image>().color=new Color(.025f,.055f,.075f,.96f);
        ...
        private void Update()
        {
            ...
            if(team)Rect((RectTransform)resources.transform,12,52,296,48);
            ...
            if(team)((RectTransform)panel.transform).sizeDelta=new Vector2(320,278);else LayoutContext();
        }
        private void LayoutContext()
        {
            // Fixed readable type sizes; only visible content consumes vertical space.
            float y=52;
            y=PlaceContent(resources,y)+8;
            if(centralSource.gameObject.activeSelf){PlacePair(centralSource,choryangSource,y);y+=38;}
            if(workEvac.gameObject.activeSelf){PlacePair(workEvac,workMedical,y);y+=38;}
            y=PlaceContent(instruction,y)+8;
            if(inside.gameObject.activeSelf||cancelTarget.gameObject.activeSelf){PlacePair(inside,cancelTarget,y);y+=38;}
            foreach(var row in teamRows)if(row.gameObject.activeSelf){Rect((RectTransform)row.transform,12,y,296,30);y+=38;}
            ((RectTransform)panel.transform).sizeDelta=new Vector2(320,y+4);
        }
        private static float PlaceContent(TMP_Text label,float y)
        {
            // Measure wrapping at the actual available width, including both disabled reasons.
            label.enableWordWrapping=true;label.overflowMode=TextOverflowModes.Overflow;
            float height=Mathf.Ceil(label.GetPreferredValues(label.text,296,0).y)+4;
            Rect((RectTransform)label.transform,12,y,296,height);return y+height;
        }
        private static void PlacePair(Button left,Button right,float y)
        {
            Rect((RectTransform)left.transform,12,y,142,30);Rect((RectTransform)right.transform,162,y,146,30);
        }
