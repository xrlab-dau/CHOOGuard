...
namespace ChooGuard.App.Mvp
{
    ...
    public sealed class MvpAgencyDispatchController : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            ...
            foreach(var target in Targets)CreatePickVolume(target.polygon,20,null,target.id);
            ApplyTeamVisibility();
        }
        private void CreatePickVolume(Vector3[] polygon,float height,string agencyId,string targetId,Vector3 pickCenter=default,Vector3 pickSize=default)
        {
            ...
            var collider=go.AddComponent<BoxCollider>();collider.size=pickSize.sqrMagnitude>.01f?pickSize:new Vector3(Mathf.Max(2,bounds.size.x),height+Mathf.Max(0,bounds.size.y),Mathf.Max(2,bounds.size.z));collider.isTrigger=false;
