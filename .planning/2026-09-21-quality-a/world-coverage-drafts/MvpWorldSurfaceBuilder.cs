...
namespace ChooGuard.Editor
{
    ...
    {
        static string Folder="Assets/ChooGuard/Art/WorldSurfaces";
        ...
        {
            Folder="Assets/ChooGuard/Art/WorldSurfaces/Build_"+Guid.NewGuid().ToString("N");Directory.CreateDirectory(Folder);AssetDatabase.Refresh();ConfigureWorldSetMaterials();
            ...
            foreach(var layer in data.layers) for(int i=0;i+2<layer.points.Length;i+=3)
            {
                ...
                var triangle=new List<Vector3>();foreach(int n in cross>0?new[]{0,2,1}:new[]{0,1,2}){var q=layer.points[i+n];triangle.Add(new Vector3(q.x*data.sourceScale,y+(layer.kind=="water"?0:Height(q.x*data.sourceScale,q.z*data.sourceScale)),q.z*data.sourceScale));}
                if(MvpSourceCoverage.Active!=null&&layer.kind!="water")MvpSourceCoverage.Active.Triangle(triangle[0],triangle[1],triangle[2],vertices,layer.kind=="land");else vertices.AddRange(triangle);
            }
            int triangleCount=0;
        ...
        public static void ConfigureWorldSetMaterials()
        {
            ...
            foreach(var spec in manifest.materials)
            {
                ...
                if(spec.textures!=null)
                {
                    ...
                    string packed="Assets/ChooGuard/Art/WorldSurfaces/"+spec.name+"-metallic-smoothness.png";
