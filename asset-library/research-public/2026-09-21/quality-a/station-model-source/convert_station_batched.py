...
model=si.sketchup.Model.from_file(str(D/'station-extracted/부산역(수정).skp'));imp=si.SceneImporter().set_filename(str(D/'station-extracted/부산역(수정).skp'));imp.context=bpy.context;imp.reuse_material=True;imp.write_materials(model.materials)
(D/'station-source-cameras.json').write_text(json.dumps([{'name':x.name,'orientation':x.camera.GetOrientation(),'fov':x.camera.fov} for x in model.scenes],ensure_ascii=False,indent=2))
...
def flush(layer):
 ...
 name='OfficialStation_'+layer+'_'+str(chunks[layer]);chunks[layer]+=1;me=bpy.data.meshes.new(name);me.from_pydata(b['verts'],[],b['faces']);me.update()
