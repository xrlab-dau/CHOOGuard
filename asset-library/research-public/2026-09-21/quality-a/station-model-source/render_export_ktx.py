...
D=pathlib.Path.cwd()/'asset-library/research-public/2026-09-21/quality-a/station-model-source';bpy.ops.wm.open_mainfile(filepath=str(D/'ktx-imported.blend'));s=bpy.context.scene
...
s.render.filepath=str(D/'ktx-source-material-preview.png');bpy.ops.render.render(write_still=True)
...
bpy.ops.wm.save_as_mainfile(filepath=str(D/'ktx-conversion-draft.blend'))
fbx=D/'OfficialKtxDraft.fbx';bpy.ops.export_scene.fbx(filepath=str(fbx),use_selection=True,object_types={'MESH','EMPTY'},axis_forward='-Z',axis_up='Y',apply_unit_scale=True,bake_anim=False,add_leaf_bones=False,path_mode='COPY',embed_textures=True)
...
(D/'ktx-conversion-manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2));print(manifest)
