...
D=pathlib.Path.cwd()/'asset-library/research-public/2026-09-21/quality-a/station-model-source';bpy.ops.wm.open_mainfile(filepath=str(D/'ktx-actual-batched.blend'));s=bpy.context.scene
...
s.render.filepath=str(D/'ktx-actual-vehicle-preview.png');bpy.ops.render.render(write_still=True)
...
bpy.ops.wm.save_as_mainfile(filepath=str(D/'ktx-actual-vehicle.blend'))
fbx=D/'OfficialKtxVehicle.fbx';bpy.ops.export_scene.fbx(filepath=str(fbx),use_selection=True,object_types={'MESH','EMPTY'},axis_forward='-Z',axis_up='Y',apply_unit_scale=True,bake_anim=False,add_leaf_bones=False,path_mode='COPY',embed_textures=True)
...
(D/'ktx-actual-vehicle-manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2));print(manifest)
