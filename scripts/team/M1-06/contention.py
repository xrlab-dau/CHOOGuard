...
BASE_A, BASE_B = lease.BASE_A, lease.BASE_B
...
def run_fixture():
    contract = lease.read_json(REPO / SOURCES[0])
    cases, registries = [], {}
    at, wf = lease.at, lease.wf
    with tempfile.TemporaryDirectory(prefix="m1-06-synthetic-") as temporary:
        root = Path(temporary)

        def registry(name):
            registries[name] = lease.LeaseRegistry(contract, root / (name + ".jsonl"))
        ...
        for name, resource in resources.items():
            r = registry(name)
            record(name + "-acquire", name, r.acquire("synthetic-a", resource, BASE_A, at(0), at(30), lease.SHA_X))
            record(name + "-conflict", name, r.acquire("synthetic-b", resource, BASE_B, at(1), at(31), lease.SHA_X),
                   False, "overlap_with_active", scope=resource)

        name, resource = "handoff", wf("Assets/Synthetic.fixture")
        r = registry(name)
        old = record("handoff-start", name, r.acquire("synthetic-a", resource, BASE_A, at(0), at(30), lease.SHA_X))
        ...
        new = record("handoff-acquire", name, r.acquire("synthetic-b", resource, BASE_B, at(2), at(32), lease.SHA_X), kind="retry")
        record("old-holder-write", name, r.write(old, "synthetic-a", 1, resource["path"], at(3), lease.SHA_Y),
               False, "transition_not_in_contract")
        record("cancel", name, r.release(new, "synthetic-b", 2, at(4)), kind="cancel")

        r = registry("unknown")
        record("unknown-holder", "unknown", r.acquire("", resource, BASE_A, at(0), at(30), lease.SHA_X),
               False, "lease_fields_unrecordable", scope=resource)

        r = registry("expiry")
        old = record("expiry-start", "expiry", r.acquire("synthetic-a", resource, BASE_A, at(0), at(10), lease.SHA_X))
        for result in r.expire_due(at(11)):
            record("expire", "expiry", result)
        record("expired-conflict", "expiry", r.acquire("synthetic-b", resource, BASE_B, at(12), at(32), lease.SHA_X),
               False, "overlap_with_expired", scope=resource)
        record("expiry-recover", "expiry", r.recover(old, "synthetic-pm", "role:pm", lease.SHA_X, at(13)))
        record("recovery-retry", "expiry", r.acquire("synthetic-b", resource, BASE_B, at(14), at(34), lease.SHA_X), kind="retry")

        for dirty in (False, True):
            name = "dirty" if dirty else "crash"
            r = registry(name)
            old = record(name + "-start", name, r.acquire("synthetic-a", resource, BASE_A, at(0), at(30), lease.SHA_X))
            record(name + "-write", name, r.write(old, "synthetic-a", 1, resource["path"], at(1), lease.SHA_Y))
            record(name + "-detected", name, r.crash_detected(old, at(2)))
            record(name + "-conflict", name, r.acquire("synthetic-b", resource, BASE_B, at(3), at(33), lease.SHA_Y),
                   False, "overlap_with_crash_suspected", scope=resource)
            record(name + "-recovery", name, r.recover(old, "synthetic-pm", "role:pm", "3" * 64 if dirty else lease.SHA_Y, at(4)),
                   not dirty, "content_changed_since_last_authorized_write" if dirty else None)
            if not dirty:
                record("crash-retry", name, r.acquire("synthetic-b", resource, BASE_B, at(5), at(35), lease.SHA_Y), kind="retry")
            else:
                record("dirty-still-blocks", name, r.acquire("synthetic-b", resource, BASE_B, at(5), at(35), lease.SHA_Y),
        ...
        raw_bytes = pipeline.encoded(raw)
        raw_path.write_bytes(raw_bytes)
        manifest = pipeline.build(raw_path, root / "private", root / "public")
        ...
        unchanged = raw_path.read_bytes() == raw_bytes and private["rawSha256"] == pipeline.digest(raw_bytes)
...
def main():
    ...
    try:
        ...
        for relative in SOURCES:
            ...
            sources[relative] = {"sha256": pipeline.digest(actual), "gitBlobSha256": pipeline.digest(committed)}
        if args.output.exists():
            raise ValueError("output exists; choose a fresh run directory")
        report, bundle = run_fixture()
