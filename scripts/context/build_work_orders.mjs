...
import {conflictReasons,validateGraph,inputProjections,conditionalNeighborhood,PHASES} from './work_graph.mjs';
...
const byId=new Map(g.items.map(x=>[x.number,x])),phases=PHASES,relations=candidateRelations(g);
...
for(const x of g.items){
 ...
 const sources=x.context.map(source),readable=c=>['published','qualified_ref'].includes(c.availability),firstRead=sources.filter(c=>readable(c)&&(c.readWhen==='prepare'||Array.isArray(c.readWhen)&&c.readWhen.includes('prepare'))).slice(0,4);
