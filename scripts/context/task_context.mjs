...
export const SECTION_FIELDS={requirements:[],relations:[],inputs:['number','hardPredecessors','inputs','oneOfInputs','guardPredicates','inputQualifiers'],writes:['number','phaseWriteScopes','artifactOutputBindings','resourceBindings','requiredLocks'],checks:['checks','acceptance','checkEvidenceMappings','stopConditions'],outputs:['outputs'],sources:['context','codePointers'],history:['truth','board','reviewNotes']};
...
export function section(order,item,name,phase,availability={files:{}}){
 ...
 if(name==='requirements')return {issue:order.issue,requirements:structuredClone(order.requirements??[]),authorization:false,rule:'Evidence-backed planning associations only. Historical/on-hold links do not authorize execution, satisfy prerequisites or establish acceptance.'};
...
export function renderBody(order,item,ref='{{CONTEXT_REF}}'){
 ...
 const reqs=(order.requirements??[]).length?order.requirements.map(r=>'`'+r.code+'`'+(r.mappingStatus==='mapped'?'':` (\`${r.mappingStatus}\` · 정책 보류, 실행 재개 아님)`)).join(', ')+' — 근거 기반 계획 대응이며 실행 선행·수용 완료가 아닙니다. 정의·근거·한계는 requirements 섹션 조회':'직접 매핑 없음; 하위 작업의 요구 매핑을 자동 상속하지 않습니다';
...
export function load(root,number,{section:name=null,body=false}={}){
 ...
 if(!body&&(!name||name==='relations'||name==='requirements'))return {order};
