...
2026-09-09 `laptop-A`에서 MSIX(Appx) 형식의 Unity Hub 3.21.1이 확인됐다. 원 작성자가 패키지 폴더 생성 시각으로 보고한 문자열은 `2026-09-08 18:44`이다. 시간대(UTC 오프셋), 로컬 시각/UTC 여부, 조회 명령과 원본 파일시스템 속성명은 미기록이다. 이 문자열을 UTC로 정규화하거나 설치 완료 시각으로 해석하지 않는다.
