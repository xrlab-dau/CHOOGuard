...
이 문서는 #69 완료 기준의 「기기별 CPU/RAM/GPU·VRAM/가용 디스크/Editor/HMD/로그인 유지 정책 표와 미확정 항목」을 채우고, 같은 작업 단위에서 수행한 laptop-A Unity 실행 파일럿을 기록한다. 관측 일자는 원 작성자 보고의 2026-09-09이며, 세부 시각과 시간대(UTC 오프셋)는 미기록이다. 아래 과거 날짜·시각도 별도 표기가 없는 한 시간대가 확인되지 않았으므로 UTC 또는 한국 표준시로 임의 변환하거나 사건의 선후 관계를 판정하는 근거로 쓰지 않는다.
...
아래 기기 사양표의 VRAM 행은 원 출처의 단위 표기를 보존했다. `laptop-A`의 9099 MB / 8388 MB는 원 작성자가 옮긴 Unity 보고값이고, `school-pc`의 8192 MiB는 PM 기록의 보고값이다. MB와 MiB를 환산하지 않았다. 각 보고값의 계산 방식과 공유 메모리 예산·전용 메모리 용량의 의미도 다르므로 수치 크기를 직접 비교하지 않는다.

가용 디스크 행의 `44.9 GiB / 231.2 GiB (파일럿 후)` 역시 원 작성자 표기를 보존했다. 각 수치의 조회 속성명과 조회 명령은 기록에 명시되지 않았으므로, 여유/전체 또는 사용/전체라는 의미를 후속 검토자가 확정하지 않는다. 원 작성자의 정제 receipt에서 두 수치의 의미를 확인해야 한다.
...
위 표는 원 작성자의 실행 보고이며 **독립 재실행 PASS가 아니다**. 이 문서에는 검토자가 재열람할 수 있는 laptop-A 정제 로그·XML·실행 명령·산출물 hash를 묶은 receipt 링크가 없다. 원자료가 없다는 뜻은 아니지만, Git 추적 밖 보존을 서술한 것만으로 독립 검수가 완료되지는 않는다.
...
원 작성자는 2026-09-09 `laptop-A`에서 MSIX(Appx) 형식의 Unity Hub 3.21.1을 확인했다고 보고했다. 원 작성자가 패키지 폴더 생성 시각으로 보고한 문자열은 `2026-09-08 18:44`이다. 시간대(UTC 오프셋), 로컬 시각/UTC 여부, 조회 명령과 원본 파일시스템 속성명은 미기록이다. 이 문자열을 UTC로 정규화하거나 설치 완료 시각으로 해석하지 않는다.
...
MSIX 확인 보고와 별개로, 원 작성자는 2026-09-09 `laptop-A`에서 사용자 결정으로 NSIS machine 범위 설치 1벌만 남겼으며, 공식 배포 URL의 설치본에 SHA256 대조·Unity Technologies 코드 서명 확인을 수행하고 관리자 승인으로 설치했다고 보고했다. 두 보고 사이의 제거·전환 절차와 정확한 순서는 재열람 가능한 근거로 확인하지 못했다. 사용자 경로 설치나 권한 우회는 하지 않았다는 서술 역시 원 작성자 보고이다. 원 작성자는 Hub가 기존 Editor 6000.3.23f1을 인식한 것을 근거로 「Unity Hub 부재가 학교 인계 절차에 주는 영향」이 해소됐다고 판단했다. 그러나 해당 설치본의 정제 영수증(receipt)·서명 검사 출력과 Hub 관측 근거는 이 문서 검토에서 재열람하지 못했으므로, 검토자의 확인이나 현재 기기 상태로 취급하지 않는다. 보고된 인식 결과가 유지된다면 인계 문서의 「Unity Hub에서 저장소 루트를 6000.3.23f1로 연다」 절차를 적용할 수 있지만, 해당 절차의 재현은 이 후속 문서 검토에서 독립 검증되지 않았다.
