...
- **탐색과 실행 권한 분리:** Graphify는 코드·출처의 탐색용 투영이다. 탐색 간선은 PM의 단계별 dependency authority나 새로운 선행·배정·수용을 만들지 않는다. canonical 입력·OR guard와 실제 claim/증거 조건은 별도로 유지한다.
...
자동 AAA/검수·REWORK 루프는 시작하거나 재개하지 않는다. 기존 익명 A/B, 검수 라운드, AAA 상태는 해당 hash·기준의 이력이며 현재 자동 실행 지시가 아니다. PM이 별도로 승인한 한정 검토만 그 범위·예산·중단 조건에서 수행하고 미달·미실행·권한 차단을 정직하게 반환한다. 보호 브랜치/CODEOWNER/CI 조건은 유지하며 기준을 낮추거나 스스로 승인하지 않는다. 환경·장소·고사양 소유나 개인 선배정은 작업 개시 조건이 아니며 기존 근거 있는 claim만 보존한다.
