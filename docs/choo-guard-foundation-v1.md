...
실제 환경과 비상대응 매뉴얼에 최대한 충실한 3D 게임이 제품 방향이다. 2026-09-07 후속 사용자 결정으로 flat 고정을 폐기했다. 공개 사진과 승인된 자료의 범위 안에서 공간 구조·설비·재질·조명을 현실적으로 저작하며, 관찰과 추정 치수를 구분한다. Foundation은 확정 자료 수신 전 교체 가능한 합성 맵과 절차 초안으로 게임을 개발한다. 합성 경로 완성이 MAP-01 또는 최종 P1 촬영·처리 이력의 충족을 의미하지 않는다.
