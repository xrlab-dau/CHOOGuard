...
`feature/foundation-starter`의 `develop` 대상 PR로 코어·Unity 호스트·flat 자산·상시 현장 시뮬레이션·생성 장면·문서·코레일 질의서를 전달한다. PR과 현재 원격 커밋은 GitHub #54/#60에서 확인한다. 병합 전에는 해당 feature 브랜치를 사용한다.
...
Git LFS 실제 파일 23개(22 FBX와 Blender 원본)가 필요하다. `git lfs fsck`로 확인한다. 이 절차는 저장소 접근 권한이 있는 개발 환경용이며 제한된 기관망 반입 조건은 코레일 회신을 따른다.
...
- 자산: 단색·무광·flat 법선의 22종 FBX, Blender 원본과 재생성 스크립트. 원본의 저장된 개인 폴더 경로 제거.
...
현재 기본 경험·구현: [상시 현장 훈련](choo-guard-open-world-training.md). 로컬 검증 범위: [상시 현장 영수증](evidence/foundation/2026-09-07-open-world.json). 전달 전 자산/문서 검수: [게시 준비 기록](evidence/foundation/2026-09-07-publication-review.json).
