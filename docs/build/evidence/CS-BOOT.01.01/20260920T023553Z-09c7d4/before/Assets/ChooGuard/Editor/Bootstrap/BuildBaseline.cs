    ...
    public static class BuildBaseline
    {
        ...
        private static void BuildFromArguments(string[] args)
        {
            ...
            try
            {
                ...
                var built = BuildPipeline.BuildPlayer(new BuildPlayerOptions { scenes = new[] { BootstrapValidator.ScenePath }, target = request.Target, locationPathName = request.Output, options = BuildOptions.Development });
                if (built == null) throw new InvalidOperationException("BuildPlayer returned no BuildReport");
                if (built.summary.result != BuildResult.Succeeded)
                    throw new InvalidOperationException("BuildReport=" + built.summary.result);
                receipt.outputs = built.GetFiles().Where(f => File.Exists(f.path)).Select(f => new FileHash { path = Path.GetFullPath(f.path), sha256 = HashFile(f.path) }).ToArray();
                receipt.buildStatus = "SUCCEEDED";
                receipt.reason = "BuildReport=" + built.summary.result + "; Player execution not performed";
            }
            catch (Exception exception)
            {
                receipt.buildStatus = "FAILED";
                receipt.reason = exception.ToString();
                SaveReceipt(receipt);
                throw;
            }
