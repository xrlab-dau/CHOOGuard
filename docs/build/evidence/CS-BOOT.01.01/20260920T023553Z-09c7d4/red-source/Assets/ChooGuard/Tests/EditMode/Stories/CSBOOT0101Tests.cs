...
namespace ChooGuard.Tests.EditMode.Stories
{
    public class CSBOOT0101Tests
    {
        ...
        {
            Assert.That(Application.platform, Is.EqualTo(RuntimePlatform.OSXEditor), "This local no-Windows-host oracle requires the observed Mac host");
            var root = BuildFixture();
        ...
        public void TmpStaticWhitelist_HasDetachedSourcesAndClosedDependencies()
        {
            ...
            Assert.That(Directory.GetFiles("Assets/ChooGuard", "*", SearchOption.AllDirectories).Where(x => new[] { ".ttf", ".otf", ".ttc" }.Contains(Path.GetExtension(x).ToLowerInvariant())), Is.Empty);
