    ...
    public static class BuildBaseline
    {
        ...
        private static void RecordBuildOutcome(BuildReceipt receipt, Func<BuildResult?> build, Func<string[]> reportedFiles)
        {
            try
            {
                ...
                receipt.outputs = reportedFiles().Where(File.Exists).Select(path => new FileHash { path = Path.GetFullPath(path), sha256 = HashFile(path) }).ToArray();
                receipt.buildStatus = "SUCCEEDED";
                receipt.reason = "BuildReport=" + result.Value + "; Player execution not performed";
            }
