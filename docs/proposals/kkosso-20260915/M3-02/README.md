...
기존 설명에 따라 참고 항목이 연결된 구역은 3개이며, 나머지 10개에는 이번에 선택한 출처가 없다. 13개 구역 모두 취득 완료 수는 0이다. 출처의 권리·개인정보·원본 식별 정보는 아직 확정되지 않았으므로, 이 단계의 허용 범위는 링크를 가리키는 기록으로 제한한다. 기존 문서의 게시/수정일은 촬영일이 아니며 이번에 다시 검증하지 않았다.
