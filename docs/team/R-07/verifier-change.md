...
판정 코드: `missing_path` · `empty_file` · `hash_drift` · `unexpected_file` · `required_glob_unmatched` · `scope_changed`. 모든 발견을 한 번에 보고한다.
...
`docs/team/R-07/proposals/P1-verify_toolchain-policy-baseline.diff` (sha256 `15eb982a5e6b71f97b59fe32ee21cb9d3d022fb42b59d3ed5e8d52dba73a0f41`)
...
| 수용 기준 | 이 후보의 근거 | 상태 |
|---|---|---|
| 누락·빈 파일·정책 hash drift·대상 부재를 관측 가능한 nonzero/state로 거부 | 대조기: 시험 15/15, 실제 worktree E4f–E4l (exit 1 ×4, exit 2 ×3). 검증기 통합: P1 격리 실행 E5 | 대조기는 충족. **정본 검증기 자체는 P1 통합 전까지 미충족** |
