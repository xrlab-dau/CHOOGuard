...
`approved/rejected` 결정에는 공백이 아닌 문자를 포함하는 `decisionRef`가 필요합니다. `unknown/pending`은 아직 참조가 없으면 `null`을 허용합니다. 빈 문자열·공백만 있는 참조는 모든 상태에서 거부합니다.
...
POSIX에서는 비공개 디렉터리를 `0700`, 모든 출력 파일을 `0600`으로 생성합니다. umask가 더 제한할 수 있으며 내용 기록 후 chmod로 권한을 좁히지 않습니다. Windows에서는 POSIX mode가 ACL을 대신하지 않으므로 기존 접근 통제된 부모 디렉터리 조건을 유지합니다. 승인 schema의 별도 회귀시험은 `jsonschema` 시험 의존성이 있는 환경에서 `python scripts/team/M1-05/test_record_schema.py`로 실행합니다.
