...
| 13 | 수동 동작 확인 (작업 지시 밖 추가 확인) | 사람이 일반 실행본을 키보드·마우스로 조작 | ✅ 약 5분 직접 조작해 이동·시점·상호작용, 사건 발생·관측, 경로 선택·통로 통과 동작을 확인. 예외 없음 |
...
- **수동 동작 확인:** 짧은 동작 확인만 했고, 실행본에서 인솔·집결·대응 완료까지 조작한 완주 확인은 하지 않았다(해당 흐름은 PlayMode 시험으로 확인).
