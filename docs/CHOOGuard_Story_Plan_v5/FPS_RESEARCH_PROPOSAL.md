...
**2026-09-25 최신 설계·우선순위:** 현재 제품은 **FPS 기반 비상상황 대응 시뮬레이션 게임**이다. [정밀 설계](design/fps-ai-20260925/DESIGN.md), [자율 NPC·JEV 동적 미래](design/fps-ai-20260925/NPC_SCENARIO.md), [OSS 적용](design/fps-ai-20260925/OSS_INTEGRATION.md), [구현 순서·수용 기준](design/fps-ai-20260925/EXECUTION_PLAN.md), [사용자 결정·후속 정정 원장](../CHOOGuard_FPS_Prior_Research_20260925/DECISIONS_AND_DEVELOPMENT_STRUCTURE.md)을 따른다. 인간 1명이 정비·청소·승무·역무를 전환하며, 별도 튜토리얼과 수백 자율 NPC의 본게임을 갖춘다. 변하는 현재 상태·인간/NPC 개입에 따라 JEV 추론과 코드 소유 인과 규칙을 반복해 다음 전개·조건부 미래를 즉시 다시 구성한다. NPC도 목표·계획·관측·기억에 따라 자발적으로 행동·재계획한다. JEV는 내장 자율 에이전트나 자유 생성 모델이 아니므로 runtime·별도 대화·외부 기억 및 정책학습 연구를 분리한다.

**아래 본문은 9월 21일 조사·제안 이력이며 최신 요구를 제한하지 않는다.** 단일 역무 역할, 고정 사건/미래 대본, 정해진 사건 pack 선택, JEV를 불확실한 값의 보조나 행동 선택에만 한정하는 해석은 위 최신 설계로 대체된다. 옛 학습 모듈 제외 문구로 이후 확정한 별도 튜토리얼·NPC 기억/정책학습 연구를 취소하지 않는다. 원래 출처 관찰·Jev 호출 결과는 그대로 보존하며, 최신 설계도 게임 구현 완료를 뜻하지 않는다. v5 활성 실행 장부와 병행 모델링 작업의 소유권은 변경하지 않는다.
