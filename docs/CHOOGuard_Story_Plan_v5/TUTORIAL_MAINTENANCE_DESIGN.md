...
### 그 3 건의 처리 (2026-09-25)

**점검표 확인 시 상호작용 끊김 — 고쳤다.** 점검표의 콜라이더를 되살렸다. 판독면을 가릴까 봐
지웠던 것인데, 그 탓에 붙은 점검표를 보려고 고개를 숙이면 설비를 놓쳤다. 점검표는 부착 지점에
붙은 설비의 일부이므로 그것을 볼 때도 설비를 겨눈 것이어야 한다. 가림은 높이로 푼다 —
점검표 `y=.26`, 판들 `.55`·`.88` 로 세로가 겹치지 않는다.

**기술자에게 몸 — 대역을 넣었다.** `TechnicianPresence` 가 진입 지점에서 작업 지점으로 이동하고,
작업 중 머물고, **플레이어가 확인할 때까지 현장에 남는다.** 확인해야 떠난다 — 그래야 "입회" 가
성립한다. `TechnicianDispatch` 에는 읽기 전용 `StageProgress01` 만 더했고 상태 기계는 그 값을
보지 않는다. 남은 시간을 초로 보이지 않는 이유는 시간 초과 실패 조건이 없기 때문이다.
**캡슐 대역이므로 이것으로 G9 의 관찰 가능성을 통과로 보지 않는다.**

**설비 3 개에 설 자리가 없다 — 고치지 않았다.** 생성기에 상시 검증을 넣어 12 개를 전부 쟀다.
`BSN-CONC-FE-001`·`-002`·`-008` 에 설 자리가 없고, `-002` 는 서른 자리가 전부 "바닥 없음"
(아래 8m 까지 바닥이 없다) — 건물 바닥 밖 좌표다. 좌표를 옮기면 NFTC 101 보행거리·구획 산정
근거가 깨지고, 덮어야 할 구획을 여전히 덮는지 검증할 방법이 없다. 실제 메시에 대해 배치를
다시 푸는 별건이 필요하다. 지금은 생성기가 매번 경고하므로 조용히 넘어가지는 않는다.

### 같은 실수를 하루에 네 번 했다

값을 계산 하나로 고정했다가 실제 구조물과 부딪힌 경우다 — 판독면 방향으로 설 자리 계산(벽에 박힘),
중력에 맡긴 착지(바닥 아래로 빠짐), 시작 지점 정면 고정(정면 네 자리 전부 실패), 기술자 작업 지점
좌우 고정(벽에 반쯤 박힘). 전부 **"재고 나서 고른다"** 로 바꿔야 풀렸다. 역사 모델이 좁고 불규칙해
좌표를 계산으로 정하면 거의 매번 무언가와 부딪힌다. **설 자리 없는 유닛 3 개도 같은 원인이다.**
