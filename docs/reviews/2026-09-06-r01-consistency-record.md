...
- **나머지 다섯 단위(M1-05·M1-06·M1-07·M2-01·M2-04): 합집합 채택.** §1.1 표의 제안 선행을 확정 선행으로 삼는다. M2-04는 그래프의 `M2-03` 간선이 중복이므로 백로그(M2-02, M4-01)를 그대로 둔다. 다섯 단위 모두 현재 `Blocked` 상태는 이 결정으로 바뀌지 않는다.
- **M4-01: 안 A 채택.** 선행 없음(`Ready`)을 확정한다. 조건으로 fixture의 `행동 ID`·`대상 Anchor` 열은 M2-02 확정 시 재검증 대상으로 표시한다. 이슈 #32의 `Blocked` 근거가 해소되므로 `Ready`로 전환한다.
...
1. ~~PM이 §1.1의 여섯 단위 선행 결정을 기록한다.~~ 완료. §1.2 참조. 이슈 #32를 `Ready`로 전환한다.
