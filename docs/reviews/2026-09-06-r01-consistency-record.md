...
초판은 이슈가 지목한 여섯 단위만 대조했다. 독립 검토(adversarial ADV-1)가 더 많은 차이를 지적해 **개별 노드로 모델링된 전체 단위를 기계적으로 전수 비교**했다. 결과는 여섯이 아니라 **열 건**이다.
...
일치하는 단위는 M0-02b·M1-01·M1-02·M1-04·M2-02·M2-03이다.

M3-01~04, M4-02~04, M5-01~05는 **비교 대상에서 제외한다.** 그래프 §1이 이들을 `M3-01~04 맵 파이프라인`, `M4-02~04 시나리오·검증`처럼 점선 집계 노드로 묶어 개별 선행을 모델링하지 않기 때문이다. 집계 노드의 부재를 "그래프가 선행을 누락했다"고 읽으면 안 된다. 이 단위들의 선행은 백로그가 단일 출처다.
...
| 대상 | 조치 | 성격 |
|---|---|---|
| 요구사항 SAF-6 / 부트스트랩 SAF-6·SAF-7 | `forbidden_workspace_files` 검사 추가. `git ls-files` 대신 작업본 전체를 걸어 미추적·ignored 입력도 검사하고 필수 검사에 등록 | RED → GREEN |
| 요구사항 SAF-7 / 부트스트랩 SAF-6 | `policy_hash_scope` 검사와 `REQUIRED_POLICY_GLOBS` 추가. 필수 정책 파일 누락이 `required_ok`를 실패시킨다 | RED → GREEN |
...
RED 18개 중 2실패 → GREEN 18/18 → 커버리지 2건 추가 → 20/20. 후속 단위 역시 독립 재검토를 받지 않았으며, 이 표는 조치 기록이지 승인이 아니다.
